...
# add parameters
parser.add_argument('--mean_window_size', type=int, default=10, help='Mean filter window size')
parser.add_argument('--median_window_size', type=int, default=10, help='Median filter window size')
...
parser.add_argument('--std_window_size', type=int, default=10, help='Standard deviation filter window size')
...
mean_window_size = args.mean_window_size
NS_copy = np.copy(median_NS)
mean_NS = np.convolve(NS_copy, np.ones(mean_window_size) / mean_window_size, mode='same')

plt.subplot(333)
plt.title('median + mean | ws = ' + str(median_window_size) + "," + str(mean_window_size))
...
for i in range(std_window_size // 2, len(mean_NS) - std_window_size // 2):
    window = mean_NS[i - std_window_size // 2:i + std_window_size // 2 + 1]
    std_dev = np.std(window)  # 计算窗口内数据的标准差
    if abs(mean_NS[i] - np.mean(window)) > threshold * std_dev:
        NS_copy[i] = np.mean(window)  # 将离群值替换为窗口内数据的均值

plt.subplot(334)
plt.title('md+mn+std | ws = ' + str(std_window_size) + ' | a = ' + str(threshold))
...
NS_copy = np.copy(denoised_NS)
wavelet_NS = wavelet_noising(NS_copy, threshold)
plt.subplot(336)
plt.title('nn + wavelet | a = ' + str(threshold))
...
plt.subplot(339)
plt.title(f'nn + tdf | ws={model_window_size} | r={ratio}')
